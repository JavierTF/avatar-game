...
describe('Bola verde — flujo completo end-to-end', () => {
    ...
    it('spawn → grab → drag (4 puntos) → drop válido → permanece estática indefinidamente', () => {
        ...
        // 5) PERSISTE ─ aunque pase mucho tiempo y haya rojas alrededor,
        // la bola dropeada permanece visible (sin destrucción por rojas).
...
describe('Bola verde — la bola dropeada NO se destruye con rojas', () => {
    const PLAYER = { x: 0, y: 1.6, z: 0 };
    ...
    it('una roja superpuesta sobre el muro NO lo elimina', () => {
        const bm = new BallManager(makeScene(), STD_CONFIG, new Difficulty(1));
        const w = placeWall(bm, 0, 0.2, -1);
        const red = bm.spawn('red', PLAYER);
        red.mesh.position.set(0, 0.2, -1);
        red.velocity.set(0, 0, 0);
    ...
    it('múltiples muros sobreviven con rojas alrededor', () => {
        const bm = new BallManager(makeScene(), STD_CONFIG, new Difficulty(1));
    ...
    it('onWallHit no existe (callback eliminado)', () => {
        const bm = new BallManager(makeScene(), STD_CONFIG, new Difficulty(1));
        expect(bm.onWallHit).toBeUndefined();
    });
});
